...
function setupJuniorSeniorToggle() {
  const heroText = document.querySelector(".hero-text");
  const span = document.getElementById("junior-senior");
  if (!heroText || !span) return;

  document.addEventListener("selectionchange", () => {
    const selection = window.getSelection();
    if (selection.rangeCount === 0) {
      if (span.textContent === "Senior") {
        span.textContent = "Junior";
      }
      return;
    }
    ...
    // Check if there is an active selection and it starts or ends inside the hero-text paragraph
    const isSelectingHero = selectedText.length > 0 && 
      (heroText.contains(range.startContainer) || heroText.contains(range.endContainer));

    if (isSelectingHero) {
      if (span.textContent === "Junior") {
        const selectionStartsInSpan = span.contains(range.startContainer);
        const selectionEndsInSpan = span.contains(range.endContainer);
        
        span.textContent = "Senior";
        
        // If the user's selection range was inside the word "Junior", adjust it to "Senior"
        const textNode = span.firstChild;
        if (textNode) {
          const newRange = document.createRange();
          if (selectionStartsInSpan && selectionEndsInSpan) {
            newRange.selectNodeContents(span);
          } else if (selectionStartsInSpan) {
      ...
      // Revert if selection is cleared or does not contain "Senior" anymore
      if (span.textContent === "Senior" && !selectedText.includes("Senior")) {
        span.textContent = "Junior";
      }
    }
  });
